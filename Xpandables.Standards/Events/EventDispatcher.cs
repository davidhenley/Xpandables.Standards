...
namespace System
{
    ...
    public sealed class EventDispatcher : IEventDispatcher
    {
        ...
        async Task IEventDispatcher.DispatchAsync<TEvent>(TEvent @event, CancellationToken cancellationToken)
        {
            ...
            var taskEvents = from handler in _serviceProvider.GetServices<IEventHandler<TEvent>>()
                             select handler.HandleAsync(@event, cancellationToken);

            await Task.WhenAll(taskEvents).ConfigureAwait(false);
        }

        async Task IEventDispatcher.DispatchAsync(IEvent @event, CancellationToken cancellationToken)
        {
            if (@event is null) throw new ArgumentNullException(nameof(@event));
