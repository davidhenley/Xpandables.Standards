...
namespace SimpleInjector.Internals
{
        ...
        IContainerControlledCollection, IReadOnlyList<TService>
    {
        ...
        public bool AllProducersVerified => producers.All(lazy => lazy.IsValueCreated);

        public int Count => producers.Count;
        ...
        public TService this[int index]
        {
            get
            {
                var producer = producers[index].Value;
        ...
        public int IndexOf(TService item)
        {
            ...
            {
                InstanceProducer producer = producers[index].Value;
        ...
        bool ICollection<TService>.Contains(TService item) => IndexOf(item) > -1;
        ...
        {
            producers.Add(ToLazyInstanceProducer(item));
        }

        KnownRelationship[] IContainerControlledCollection.GetRelationships() => (
            from producer in producers.Select(p => p.Value)
        ...
        private LazyEx<InstanceProducer> ToLazyInstanceProducer(ContainerControlledItem item) =>
            item.Registration != null
                ...
                : new LazyEx<InstanceProducer>(() => GetOrCreateInstanceProducer(item));
        ...
        private InstanceProducer GetOrCreateInstanceProducer(ContainerControlledItem item)
        {
            ...
            InstanceProducer? producer = GetExplicitRegisteredInstanceProducer(implementationType);
            ...
            if (producer == null)
            {
                producer = GetInstanceProducerThroughUnregisteredTypeResolution(implementationType);
            }
            ...
            if (producer == null)
            {
                return CreateNewExternalProducer(item);
            }
            ...
            return new InstanceProducer(typeof(TService),
                new ExpressionRegistration(producer.BuildExpression(), container));
        }
        ...
        {
            var producer = container.GetRegistrationEvenIfInvalid(
                implementationType,
        ...
        private InstanceProducer CreateNewExternalProducer(ContainerControlledItem item)
        {
            ...
            Lifestyle lifestyle = container.SelectionBasedLifestyle;

            // This producer will be automatically registered as external producer.
            return lifestyle.CreateProducer(typeof(TService), item.ImplementationType, container);
        }
