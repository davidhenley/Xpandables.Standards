...
namespace SimpleInjector.Internals
{
    ...
    internal static partial class CompilationHelpers
    {
        ...
        private sealed class PerObjectGraphOptimizableRegistrationFinder : ExpressionVisitor
        {
            ...
            {
                Registration? registration = GetScopedRegistration(node);

                if (registration != null)
                {
                    perObjectGraphRegistrations.Add(new OptimizableRegistrationInfo(registration, node));
                }
            ...
            private Registration? GetScopedRegistration(MethodCallExpression node)
            {
                ...
                if (registration != null && object.ReferenceEquals(registration.Container, container)
        ...
        private sealed class ObjectGraphOptimizerExpressionVisitor : ExpressionVisitor
        {
            ...
            {
                var registration =
                    registrationsToOptimize.FirstOrDefault(r => r.OriginalExpression == node);
        ...
        private sealed class OptimizableLifestyleInfo
        {
                ...
                IEnumerable<OptimizableRegistrationInfo> registrations)
            {
                ...
                Variable = Expression.Variable(typeof(LazyScope));
        ...
        private sealed class OptimizableRegistrationInfo
        {
                ...
                MethodCallExpression originalExpression)
            {
                ...
                lazyScopeRegistrationType = typeof(LazyScopedRegistration<>)
                    .MakeGenericType(Registration.GetType().GetGenericArguments());
            }
            ...
            internal ParameterExpression Variable
            {
                get
                {
                    if (variable == null)
                    {
                        variable = Expression.Variable(lazyScopeRegistrationType);
                    }
            ...
            internal Expression LazyScopeRegistrationGetInstanceExpression =>
                Expression.Call(
                    Variable,
                    lazyScopeRegistrationType.GetMethod("GetInstance"),
                    Expression.Property(LifestyleInfo!.Variable, "Value"));
        }

        [DebuggerDisplay(
            nameof(ExpressionInfo.TotalCost) + ": {" + nameof(ExpressionInfo.TotalCost) + "}, " +
            nameof(ExpressionInfo.Count) + ": {" + nameof(ExpressionInfo.Count) + "}, " +
            nameof(ExpressionInfo.TreeSize) + ": {" + nameof(ExpressionInfo.TreeSize) + "}, " +
            nameof(ExpressionInfo.Node) + ": {" + nameof(ExpressionInfo.Node) + "}")]
        private sealed class ExpressionInfo
        {
            public ExpressionInfo(Expression node) => Node = node;

            public Expression Node { get; }
            public int Count { get; set; }
            public int TreeSize { get; set; }
            public int TotalCost => Count * TreeSize;
        }
        ...
        private sealed class NodeSizeCalculator : ExpressionVisitor
        {
            ...
            private ExpressionInfo GetInfo(Expression node)
            {
                ...
                if (nodes.TryGetValue(node, out info))
                {
                    return info;
                }
                else
        ...
        private sealed class NodeReplacer : ExpressionVisitor
        {
            ...
            public override Expression Visit(Expression node) =>
                object.ReferenceEquals(node, oldNode)
                    ? newNode
                    : base.Visit(node);
        }
    }
